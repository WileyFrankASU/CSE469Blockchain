...
# calculates the sha256 hash of a block
    def calculate_hash(self, block):
        ...
        block_data = struct.pack(
            ...
            block.prev_hash.encode("utf-8").ljust(32, b'\0') if isinstance(block.prev_hash, str) else block.prev_hash.ljust(32, b'\0'),
            block.timestamp,
            bytes.fromhex(block.case_id).ljust(32, b'\0') if isinstance(block.case_id, str) else block.case_id.ljust(32, b'\0'),
            bytes.fromhex(block.item_id).ljust(32, b'\0') if isinstance(block.item_id, str) else block.item_id.ljust(32, b'\0'),
            block.state.encode("utf-8").ljust(12, b'\0') if isinstance(block.state, str) else block.state.ljust(12, b'\0'),
            block.creator.encode("utf-8").ljust(12, b'\0') if isinstance(block.creator, str) else block.creator.ljust(12, b'\0'),
            block.owner.encode("utf-8").ljust(12, b'\0') if isinstance(block.owner, str) else block.owner.ljust(12, b'\0'), # fields are padded to fixed sizes
            len(block.data),
        )
    ...
    def add(self, case_id, item_ids, creator, password):
        ...
        #checks for duplicates in all - > mainly for the inital block
        for item_id in item_ids:
            encrypted_item_id = encrypt(str(item_id).strip()).hex()

            
            if any(block.item_id.rstrip('0').rstrip() == encrypted_item_id.rstrip('0').rstrip() for block in self.chain):
                print(f"Duplicate found for item_id: {item_id}")
                raise ValueError(f"Duplicate Item ID {item_id} found or already exists.")
            if encrypted_item_id in processed_item_ids:
                print(f"Duplicate found in session for item_id: {item_id}")
                raise ValueError(f"Duplicate Item ID {item_id} found in this session.")

            processed_item_ids.add(encrypted_item_id)

        # create the genesis block if the blockchain is empty
        if not self.chain:
            print("Blockchain is empty. Creating block.")
            genesis_block = Block(
                prev_hash=b"\x00" * 32,
                timestamp=get_timestamp(),
                case_id=encrypt(case_id).hex(),
                item_id=encrypt(str(item_ids[0]).strip()).hex(),
                state="INITIAL",
                creator=creator,
                owner="",
                data=""
            ...
            print(
                f"Added item: {item_ids[0]}\n"
                f"Status: INITIAL\n"
                f"Time of action: {datetime.fromtimestamp(genesis_block.timestamp, timezone.utc).isoformat()}Z"
            )
            item_ids = item_ids[1:]  # skip the first item_id as it is already added

        # for each item_id, ensure it is not empty or a duplicate and then add it to the blockchain
        for item_id in item_ids:
            ...
            for block in self.chain:
                existing_item_id = block.item_id.rstrip('0').rstrip()  # strip padding or null bytes
                if encrypted_item_id == existing_item_id:
                    print(f"Duplicate found for item_id: {item_id_str}")
                    raise ValueError(f"Duplicate Item ID {item_id_str} found or already exists.")

            # check for duplicates in the current session
            if encrypted_item_id in processed_item_ids:
                print(f"Duplicate found in session for item_id: {item_id_str}")
                raise ValueError(f"Duplicate Item ID {item_id_str} found in this session.")
    ...
    def write_block(self, block):
        ...
        with open(self.path, "ab") as f:
            ...
            packed_block = struct.pack(
                ...
                block.state.encode("utf-8").ljust(12, b'\0') if isinstance(block.state, str) else block.state.ljust(12, b'\0'),
                block.creator.encode("utf-8").ljust(12, b'\0') if isinstance(block.creator, str) else block.creator.ljust(12, b'\0'),
                block.owner.encode("utf-8").ljust(12, b'\0') if isinstance(block.owner, str) else block.owner.ljust(12, b'\0'),
                len(block.data),
            )
    ...
    def checkout(self, item_id, password):
        ...
        block = next(
            (b for b in reversed(self.chain) if b.item_id.rstrip('0').rstrip() == encrypted_item_id.rstrip('0').rstrip()),
            None,
        )
        if not block:
